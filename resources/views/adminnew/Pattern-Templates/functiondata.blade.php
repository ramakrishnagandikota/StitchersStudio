                            ...
                            @foreach($function->factors as $factor)
                                ...
                                @if($factor->is_stitches == 0)
                                    @if($factor->pivot->factor_uom == 'in')
                                        ...
                                        <div class="col-lg-6 snippet-accordion factor_column">
                                            <div class="row theme-row m-b-10">
                                                <div class="card-header accordion p-1 col-lg-12 col-sm-12" data-toggle="collapse" data-target="#child-accord1factor_column1{{ $dataCount }}{{$factor->pivot->factor_uom}}">
                                            ...
                                            <div class="card-block collapse" id="child-accord1factor_column1{{ $dataCount }}{{$factor->pivot->factor_uom}}">
                ...
                <!-- conditional variables here -->
                    ...
                    @if($IfConditions->count() > 0)
                        ...
                        @foreach($IfConditions as $ifcond)
                            <div class="col-md-12">
                                <div class="row">
                                    <div class="card-header m-b-5 snippet{{$dataCount}} mt-10" role="tab" id="headingOne12{{$ifcond->id}}{{$dataCount}}{{$function->id}}" style="background-color: #dedddd;width:100%;">
                                        <a data-toggle="collapse" data-parent="#accordionEx" href="#collapseOne12{{$ifcond->id}}{{$dataCount}}{{$function->id}}" aria-expanded="true" aria-controls="collapseOne1">
                                            <h5 class="mb-0">Conditional Variable : {{ $ifcond->condition_variable }}</h5>
                                            <i class="fa fa-caret-down pull-right micro-icons"></i>
                                        </a>
                                    </div>

                                    <div id="collapseOne12{{$ifcond->id}}{{$dataCount}}{{$function->id}}" class="collapse snippets mb-10 col-md-12 row" role="tabpanel" aria-labelledby="headingOne1" data-parent="#accordionEx">
                                        ...
                                        @if($ifcond->conditionalVariables()->count() > 0)
                                            @foreach($ifcond->conditionalVariables as $condvars)
                                                ...
                                                @component('adminnew.Pattern-Templates.summernote', ['condId' => $condvars->id,'k' => 0,'dataCount' => $dataCount,'l' => $ifcond->id,'outputVariables' => $function->outputVariables,'measurements' => $measurements]) @endcomponent
                                            @endforeach
                                            <br>
                                        @endif
...
$array1 = array('<span class="text-danger">[[PART-1]]</span>','<span class="text-danger">[[PART-2]]</span>','<span class="text-danger">[[PART-3]]</span>','<span class="text-danger">[[PART-4]]</span>','<span class="text-danger">[[PART-5]]</span>','<span class="text-danger">[[PART-6]]</span>','<span class="text-danger">[[PART-7]]</span>','<span class="text-danger">[[PART-8]]</span>','<span class="text-danger">[[PART-9]]</span>','<span class="text-danger">[[PART-10]]</span>','<span class="text-danger">[[PART-11]]</span>','<span class="text-danger">[[PART-12]]</span>','<span class="text-danger">[[PART-13]]</span>','<span class="text-danger">[[PART-14]]</span>');
                                    ...
                                    @for($l=1;$l<=$cond->instructions_count;$l++)
                                        <!-- <h5 class="m-t-10 text-danger">PART-{{ $l }}</h5> -->
                                            ...
                                            @component('adminnew.Pattern-Templates.summernote', ['condId' => $cond->id,'k' => $k,'dataCount' => $dataCount,'l' => $l,'outputVariables' => $function->outputVariables,'measurements' => $measurements]) @endcomponent
                                        @endfor
