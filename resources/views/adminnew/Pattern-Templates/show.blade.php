...
<div class="page-body">
    <div class="row">
        <div class="col-lg-6"><h5 class="theme-heading p-10">{{$template->template_name}}</h5></div>
        <div class="col-lg-6">
            ...
            <a href="{{ route('pattern.templates.list') }}" class="btn theme-btn pull-right btn-sm waves-effect m-b-10 m-t-10 m-r-20">Back to templates</a>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="row">
                    <div class="col-lg-6"><h5 class="p-10">Pattern Template</h5></div>
                    <div class="col-lg-6">
...
<!-- section data start -->
                ...
                <form class="form" id="description" method="POST" action="{{ route('template.pattern.add.data') }}">
                    @csrf
                    ...
                    <div class="row theme-row m-b-10">
                        <div class="card-header accordion active col-lg-12 col-sm-12" data-toggle="collapse" data-target="#PTsection1">
                            <h5 class="card-header-text">Project Information</h5><i class="fa fa-caret-down pull-right micro-icons"></i> </div>
                    </div>
               ...
               <!-- <form class="form" id="materials" method="POST" action="{{ route('template.pattern.add.data') }}">
                    ...
                    <div class="row theme-row m-b-10">
                        <div class="card-header accordion active col-lg-12 col-sm-12" data-toggle="collapse" data-target="#PTsection2">
                            <h5 class="card-header-text">Materials</h5><i class="fa fa-caret-down pull-right micro-icons"></i> </div>
                    </div>
          ...
          <!--      <form class="form" id="techniques" method="POST" action="{{ route('template.pattern.add.data') }}">
                    ...
                    <div class="row theme-row m-b-10">
                        <div class="card-header accordion active col-lg-12 col-sm-12" data-toggle="collapse" data-target="#PTsection3">
                            <h5 class="card-header-text">Techniques</h5><i class="fa fa-caret-down pull-right micro-icons"></i> </div>
                    </div>
...
<div id="sidebar" class="users p-chat-user showChat preview-box-parent">
    ...
    <div class="modal-dialog modal-sm">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn theme-outline-btn" data-dismiss="modal">Close</button>
                    <button type="submit" class="btn theme-btn displayChatbox" data-template-id="{{$id}}">Preview template</button>
                </div>
            </div>
    ...
    <script>
        $(function(){
            ...
            $('#create-section')
                ...
                $.ajax({
                    url: '{{ route("template.pattern.add.section") }}',
                    type: 'POST',
                        ...
                        if(res.status == 'error'){
                           ...
                           /* Swal.fire({
                            ...
                            for(var i = 0;i < data.length;i++){
                                if(data.length == 1){
                                    array+='<b>'+data[i].function_name+'</b> ';
                                }else if(data.length == 2){
                                    if(i == (data.length) - 1){
                                        array+='and <b>'+data[i].function_name+'</b>';
                                    }else{
                                        array+='<b>'+data[i].function_name+'</b> ';
                                    }
                                }else{
...
<script>
    $(function() {
        ...
        $(document).on('click','.deleteSection',function(){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                       url : '{{ route('pattern.template.delete.section') }}',
                       type : 'POST',
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url : '{{ route('pattern.template.delete.snippet') }}',
                        type : 'POST',
        ...
        // open chat box
        $(document).on('click','.displayChatbox',function(e) {
            ...
            $.ajax({
                ...
                url : '{{ route("pattern.template.preview") }}',
                type : 'POST',
        ...
        $(document).on('click','.delete-yarn-details',function(){
            ...
            if(confirm('Are you sure want to remove this ?')){
                ...
                if(server == 'true'){
                    $(".loading").show();
                    $.get("{{ url('adminnew/template-delete-yarn-data') }}/"+id,function (res){
                        if(res.status == 'success'){
...
function getYarnDetails(){
    $.get('{{ url("adminnew/template-get-yarn-data/".$id) }}',function(res){
...
/*
 function getAllSections(section_id){
     $.get("{{ url('adminnew/get-all-sections/'.$id) }}",function(res){
         ...
         setTimeout(function (){
             ...
             $('.username').editable({
                 type: 'text',
                 url: '{{route("update.section.name")}}', // Send ajax request with new value on /post
  ...
  function getAllSections(section_id){
     ...
     $.ajax({
         url: "{{ url('adminnew/get-all-sections/'.$id) }}",
         type: 'GET',
         beforeSend : function(){
             $(".loading").show();
         },
...
function getNewSection(){
    $.get("{{ url('adminnew/get-new-sections/'.$id) }}",function(res){
...
var pattern_template_id = $("#pattern_template_id").val();
    ...
    $.get('{{ url("adminnew/remove-added-functions") }}/'+pattern_template_id,function (res) {
...
function saveData(Data){
    ...
    function opencommentbar(sectionId) {
        ...
        $.get("{{url('adminnew/getSectionComments')}}/"+sectionId,function (res){
