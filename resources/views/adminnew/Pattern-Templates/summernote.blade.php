...
<script>
    ...
    $("#editor{{$condId}}{{$k}}{{$dataCount}}{{$l}}").on('summernote.blur', function(we, contents, $editable) {
		...
		//console.log("#editor{{$condId}}{{$k}}{{$dataCount}}{{$l}}"+ ', sid : '+sid);
        ...
        if((len > 1) && (k == 1)) {
            ...
            $(".conditional_statements_id" + sid).each(function (i) {
				...
				//console.log("#editor" + val + '' + k1 + '' + sid + '' + l);
				
                var checked = $("#sameCond1" + val + '' + k1 + '' + sid).prop('checked');
                if ((checked == true) && (i > 0)) {
                    var summernote1 = $("#editor" + val + '' + k1 + '' + sid + '' + l);
