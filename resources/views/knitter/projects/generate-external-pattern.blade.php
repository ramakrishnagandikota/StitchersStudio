...
@php 
$pimage = App\Models\Projectimages::where('project_id',$project->id)->where('image_ext','!=','pdf')->first();
@endphp
@if($pimage)
<img src="{{$pimage->image_path}}" alt="" style="width:100%; ">
@else
...
<img data-id="{{$i}}" class=" self_posted all hide sectionContent" src="{{ $pi->image_path }}">
...
@foreach($project_images as $pi)
@if($pi->image_ext == 'pdf')
    <iframe src="{{$pi->image_path}}" width="100%" height="500px"></iframe>
@endif
