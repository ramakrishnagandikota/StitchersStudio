...
<!--
<a href="{{url('knitter/generate-custom-pattern/'.$project->token_key.'/'.Str::slug($project->name))}}?again=yes" class="btn btn-theme tablike-bt-outlined waves-effect">Regenerate Pattern</a> -->
@if($project->product_id == 19)
	<a href="javascript:;" class="btn btn-theme tablike-bt-outlined waves-effect shapeUnshaped" >Regenerate Pattern</a>
@else
	<a href="{{url('knitter/generate-custom-pattern/'.$project->token_key.'/'.Str::slug($project->name))}}?again=yes" class="btn btn-theme tablike-bt-outlined waves-effect" id="generate-again">Regenerate Pattern</a>
@endif
...
<a class="btn btn-success theme-btn-sm waves-effect waves-light" style="margin-top:0px" target="_blank" href="{{url('knitter/generate-custom-pattern/'.$id.'/'.$slug.'?type=print')}}"><i class="icofont icofont-print f-20"></i></a>
...
<div class="col-md-5">
    <div class="form-group">
        ...
        <div class="row">
            <div class="col-md-12">
                <label class="col-form-label m-l-15">{{ucfirst($product->skill_level)}}</label>
            </div>
...
<div class="col-md-4">
    <div class="form-group">
        ...
        <div class="row">
            <div class="col-md-12">
                <label class="col-form-label m-l-15">For {{ $measurements->title ? ucfirst($measurements->title) : '' }}</label>
            </div>
        </div>
    </div>
</div>
...
<!--Fifth Accordion-->
<div class="row theme-row m-b-10">
    <div class="card-header border-accordion1 accordion col-lg-12" data-toggle="collapse" data-target="#section5-NC">
        <h5 class="card-header-text">Pattern-specific measurements</h5>
        <i class="fa fa-caret-down pull-right micro-icons"></i>
    </div>
</div>
<div class="card-block collapse" id="section5-NC">
    <div class="row form-group">

        @if($pdm->count() > 0)
        @foreach($pdm as $pd)
        <?php
        $name = ucfirst($pd->measurement_name);
        ?>
        <div class="col-md-4">
            <div class="form-group">
                <label class="col-form-label f-w-600"> {{str_replace('_',' ',$name)}}
                </label>
                <div class="row">
                    <div class="col-md-12">
                        <label class="col-form-label m-l-15">{{ $pd->measurement_value ? $pd->measurement_value : 'NA' }}</label>
                    </div>
...
<div class="modal fade" id="child-Modal" role="dialog">
			  ...
			  <div class="modal fade" id="shapeUnshaped" role="dialog">
<div class="modal-dialog modal-md">
  <div class="modal-content">
  
  <div class="modal-header">
		  <h5 class="modal-title">Select shaping</h5>
	  <button type="button" class="close" data-dismiss="modal">&times;</button>
	</div>
	...
	<div class="modal-body text-center">
		...
		<form>
			<div class="form-group text-left">
				<!--<label>Please select type of patterm</label>-->
				<select class="form-control" id="varient">
					<option value="">Select shaping</option>
					<option value="0">Knit body with custom shaping</option>
					<option value="1">Knit body without shaping</option>
				</select>
				<span class="red varient"></span>
			</div>
		</form>
	</div>
	...
	<div class="modal-footer">
		  <button type="button" class="btn btn-danger cancel-btn" data-dismiss="modal">Cancel</button>
		   <button type="button" id="saveRegerate" class="btn btn-success submit-btn" >Continue to Regenerate Pattern</button>
	</div>
...
<input type="hidden" id="pageUrl" value="{{url('knitter/generate-custom-pattern/'.$project->token_key.'/'.Str::slug($project->name))}}">
@endsection
...
<script src="{{asset('resources/assets/files/assets/pages/todo/todo.js')}}"></script>
...
$(document).ready(function() {
	var str = document.getElementById("pageUrl").value;	
	...
	$(document).on('click','.shapeUnshaped',function(){
	var options = {
		backdrop: 'static',
		keyboard : false
	}
	$("#shapeUnshaped").modal(options);
});
...
$(document).on('click','#saveRegerate',function(){
	var checkVarient = $("#varient").val();
	var er = [];
	var cnt = 0;
	
	if(checkVarient == ""){
		$(".varient").html('Please select shaping.');
		er+=cnt+1;
	}else{
		$(".varient").html('');
	}
	...
	var url = "{{url('knitter/generate-custom-pattern/'.$project->token_key.'/'.Str::slug($project->name))}}?again=yes&varient="+varient;
