...
<a href="{{url('knitter/generate-noncustom-pattern/'.$project->token_key.'/'.Str::slug($project->name))}}?again=yes" class="btn btn-theme tablike-bt-outlined waves-effect" id="generate-again">Regenerate Pattern</a>
...
<a class="btn btn-success theme-btn-sm waves-effect waves-light" href="{{url('knitter/generate-noncustom-pattern/'.$project->token_key.'/'.Str::slug($project->name))}}?type=print" target="_blank" style="margin-top:0px" id=""><i class="icofont icofont-print f-20"></i></a>
...
<div class="col-md-5">
    <div class="form-group">
        ...
        <div class="row">
            <div class="col-md-12">
                <label class="col-form-label m-l-15">{{ucfirst($product ? $product->skill_level : '')}}</label>
            </div>
...
<div class="col-md-4">
    <div class="form-group">
        ...
        <div class="row">
            <div class="col-md-12">
                <label class="col-form-label m-l-15"> @if($stitch_gauge) {{$project->uom == 'in' ? $stitch_gauge->stitch_gauge_inch.' sts / 1 Inches' : $stitch_gauge->stitches_10_cm.' sts / 10 Centimeters' }} @else NA @endif</label>
            </div>
        </div>
    </div>
</div>
...
<div class="col-md-4">
    <div class="form-group">
        ...
        <div class="row">
            <div class="col-md-12">
                <label class="col-form-label m-l-15"> @if($row_gauge) {{$project->uom == 'in' ? $row_gauge->row_gauge_inch.' sts / 1 Inches' : $row_gauge->rows_10_cm.' sts / 10 Centimeters' }} @else NA @endif</label>
            </div>
        </div>
    </div>
</div>
...
<div class="col-md-4">
    <div class="form-group">
        ...
        <div class="row">
            <div class="col-md-12">
                <label class="col-form-label m-l-15">@if($measurements) For {{ucfirst($measurements->title)}} @else NA @endif</label>
            </div>
        </div>
    </div>
</div>
...
<i class="fa fa-pencil-square-o"></i>
    ...
    <div class="card row outline-row m-b-10">
        <div class="card-header accordion col-lg-11" data-toggle="collapse" data-target="#accordion1">
            <h5 class="card-header-text">Pattern instructions</h5>
        </div>
        <div class="col-lg-1 m-t-15" style="position: absolute;right: 0px;">
            <i class="fa fa-caret-down pull-right micro-icons"></i>
        </div>
    </div>

    <div class="card-block collapse" id="accordion1">
        <div class="card-block non-custom-pattern">
            <div class="row">
			
               <!-- @if($product->traditionalPatternpdf()->count() > 0)
                    @foreach($product->traditionalPatternpdf as $patpdf)
                        @if($patpdf->type == 'pdf')
                            <iframe width="100%" height="1000px" src="{{ $patpdf->instructions_file
                            }}"></iframe>
                        @else
                                <h3 class="text-center"><a href="{{ $patpdf->instructions_file }}" target="_blank"
                                   style="text-decoration:underline;" download >Download file</a></h3>
                        @endif
                    @endforeach
                @endif -->
...
<div class="row m-b-10">
    ...
    <!--
<div class="col-lg-12 card panel-group" id="accordion1">
