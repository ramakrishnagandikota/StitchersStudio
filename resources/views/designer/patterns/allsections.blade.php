                                        ...
                                        <div class="row">
                                            @if($designType->functions()->count() > 0)
                                                ...
                                                <div class="col-md-5 m-b-20">
                                                    ...
                                                    <input type="hidden" class="function_name" name="function_name" value="{{ $function->id }}">
                                        ...
                                        <div class="row">
                                            ...
                                            @if($function->factors()->count() > 0)
                                                @foreach($function->factors as $factor)
                                                    ...
                                                    <input type="hidden" name="snip_factor_id" value="{{ $fac ? $fac->id : 0 }}">
                                                    ...
                                                    @else
                                                        @if($factor->pivot->factor_uom == 'in')
                                                            ...
                                                            <div class="col-lg-6 snippet-accordion factor_column">
                                                                <div class="row theme-row m-b-10">
                                                                    <div class="card-header accordion p-1 col-lg-12 col-sm-12" data-toggle="collapse" data-target="#child-accord1factor_column1{{ $allsec->id }}{{$se}}{{$factor->id}}{{$factor->pivot->factor_uom}}">
                                                                        <h5 class="card-header-text">{{ $factor->factor_name }} :</h5><i class="fa fa-caret-down pull-right micro-icons"></i> </div>
                                                                </div>
                                                                <div class="card-block collapse" id="child-accord1factor_column1{{ $allsec->id }}{{$se}}{{$factor->id}}{{$factor->pivot->factor_uom}}">
                                            ...
                                            @if($function->modifiers()->count() > 0)
                                                @foreach($function->modifiers as $modifier)
                                                    ...
                                                    <input type="hidden" name="snip_modifier_id" value="{{ $mod ? $mod->id : 0 }}">
                                                    ...
                                                    @if($modifier->is_stitches == 0)
                                                        @if($modifier->pivot->modifier_uom == 'in')
                                                            <div class="col-lg-6 snippet-accordion factor_column">
                                                                <div class="row theme-row m-b-10">
                                                                    <div class="card-header accordion p-1 col-lg-12 col-sm-12" data-toggle="collapse" data-target="#child-accord1modifier_column1{{ $allsec->id }}{{$se}}{{$modifier->id}}{{$modifier->pivot->id}}">
                                                        ...
                                                        @else
                                                            <div class="col-lg-6 snippet-accordion factor_column">
                                                                <div class="row theme-row m-b-10">
                                                                    <div class="card-header accordion p-1 col-lg-12 col-sm-12" data-toggle="collapse" data-target="#child-accord1modifier_column1{{ $allsec->id }}{{$se}}{{$modifier->pivot->modifier_uom}}">
                                                                        <h5 class="card-header-text">{{ $modifier->modifier_name }} ({{ ($modifier->pivot->modifier_uom == 'in') ? 'Inches' : 'Cm' }}):</h5><i class="fa fa-caret-down pull-right micro-icons"></i> </div>
                                                                </div>
                                                                <div class="card-block collapse" id="child-accord1modifier_column1{{ $allsec->id }}{{$se}}{{$modifier->pivot->modifier_uom}}">
                                                                    <input type="hidden" name="modifier_uom[]" value="{{$modifier->pivot->modifier_uom}}">
                                                                    @if($mod)
                                                                        <select class="form-control fill select-modifier" name="modifier_cm[]" id="modifier{{$modifier->pivot->modifier_uom}}">
                                                                            <option value="">Select {{ $modifier->modifier_name }}</option>
                                                                            @if($modifier->is_negative == 1)
                                                                                @foreach (range('-'.$modifier->max_value, $modifier->max_value) as $number) {
                                                                                @if($number > 0)
                            ...
                            @endphp
                                        @if($IfConditions->count() > 0)
                                            ...
                                            @foreach($IfConditions as $ifcond)
                                                <div class="col-md-12">
                                                    <div class="row">
                                                        <div class="card-header m-b-5 snippet{{$se}} mt-10" role="tab" id="headingOne12{{$ifcond->id}}{{$function->id}}" style="background-color: #dedddd;width:100%;">
                                                                    ...
                                                                    @if($snip->conditionalVariableOutput()->count() > 0)
                                                                        @foreach($snip->conditionalVariableOutput as $outvars)
                                                                            @if($outvars->condition_variable_id == $condvars->id)
                                                                                ...
                                                                                @component('designer.patterns.summernote', ['condId' => $condvars->id,'k' => 0,'dataCount' => $se,'l' => $outvars->id,'outputVariables' => $function->outputVariables,'measurements' => $measurements]) @endcomponent
                                                                            @endif
                                                                        @endforeach

                                                                    @else
                                        <input type="hidden" name="conditional_variables_outputs_id[]" value="0">
                                        <textarea id="editor{{$condvars->id}}0{{$se}}0" name="condition_description[]" class="form-control"></textarea>
                                        @component('adminnew.Pattern-Templates.summernote', ['condId' => $condvars->id,'k' => 0,'dataCount' => $se,'l' => 0,'outputVariables' => $function->outputVariables,'measurements' => $measurements]) @endcomponent
                                                ...
                                                <div class="col-lg-12">
                                                    ...
                                                    @foreach($snip->snippetConditionalStatements as $snipCond)
                                                        ...
                                                        <div class="card-header m-b-5 snippet{{$se}}" role="tab" id="headingOne1{{$allsec->id}}{{$snipCond->id}}{{$se}}" style="background-color: #dedddd;">
                                                            <a data-toggle="collapse" data-parent="#accordionEx" href="#collapseOne1{{$snipCond->id}}{{$k}}{{$se}}{{$function->id}}" aria-expanded="true" aria-controls="collapseOne1">
                                                                <h5 class="mb-0">
                                                            ...
                                                            <div class="row" style="position: absolute;right: 50px;z-index: 10;top: 10px;@if($k == 1) display:none; @endif" >
                                                                <div class="col-md-12">
                                                                    <div class="checkbox">
                                                                        ...
                                                                        <input type="checkbox" class="sameAsCondition sameCond1{{$se}}" id="sameCond1{{$snipCond->id}}{{$k}}{{$se}}"  value="{{$snipCond->id}}" data-tabid="{{$allsec->id}}{{$snipCond->id}}{{$se}}" data-divId="{{$snipCond->id}}{{$k}}{{$se}}" data-condid="{{$snipCond->id}}" data-sid="{{$se}}" data-k="{{$k}}" @if($checkbox->sameAsCondition == 1) checked @endif value="{{$checkbox->id}}">Same as Condition 1
                                                        ...
                                                        <div id="collapseOne1{{$snipCond->id}}{{$k}}{{$se}}{{$function->id}}" class="collapse snippets{{$se}}{{$snipCond->id}}" role="tabpanel" aria-labelledby="headingOne1" data-parent="#accordionEx">
                                                            ...
                                                            <!-- <h5 class="m-t-10 text-danger">PART-{{ $n }}</h5> -->
                                                                ...
                                                                <textarea class="hint2mention summernote m-b-10" id="editor{{$snipCond->id}}{{$k}}{{$se}}{{$n}}{{$function->id}}" name="description[]" data-condid="{{$snipCond->id}}" data-cid="{{$allsec->id}}{{$snipCond->id}}" data-sid="{{$se}}" data-k="{{$k}}" data-l="{{$n}}">{!! $instructions->description !!}</textarea>
                                                                <span class="summernote-required text-danger"></span>
                                                                @component('designer.patterns.summernote', ['condId' => $snipCond->id,'k' => $k,'dataCount' => $se,'l' => $n.''.$function->id,'outputVariables' => $function->outputVariables,'measurements' => $measurements]) @endcomponent
                                                                <?php $n++; ?>
                                                            @endforeach
                                                            <br>
                                                        </div>
