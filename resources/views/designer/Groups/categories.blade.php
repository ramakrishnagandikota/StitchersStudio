...
<button class="btn btn-sm theme-btn waves-effect waves-light pull-right m-b-10" id="addCategory">Add group type</button>
@if($category->count() > 0)
        ...
        @foreach($category as $gc)
            <tr>
                <td class="text-center">
                    <a href="#" class="category_name" data-type="text" data-pk="{{ $gc->id }}" data-url="{{ url('designer/groups/categories/update') }}" data-title="Enter category name">{{ $gc->category_name }}</a>
                </td>
...
@else
    ...
    <div class="text-center">
        There are no group types to show for this group. To add click on Add group type.
    </div>
@endif
