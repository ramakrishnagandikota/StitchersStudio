    ...
    @section('footerscript')
        ...
        <script>
            ...
            function showAllReplies(ticket_id,page){
                $("#supportReply").html('<div class="col-lg-12"><h5 class="text-center"><i class="fa fa-spin fa-spinner"></i> Loading reply`s...</h5></div>');
                //var page = $(this).attr('href').split('page=')[1];
                var jqxhr = $.get("{{ url('support/reply') }}/"+ticket_id+"?page="+page,function (res){
                   if(res.status == false){
                       $("#supportReply").html('<div class="col-lg-12"><h5 class="text-center">Unable to load replies for this ticket.<a href="javascript:;" onclick="showAllReplies({{$support->ticket_id}},1)">Click here</a> to load coments.</h5></div>');
                   }else{
                       $("#supportReply").html(res);
                       setTimeout(function(){ loadImagePlugin(); },1000);
                   }
                }).fail(function() {
                    $("#supportReply").html('<div class="col-lg-12"><h5 class="text-center">Unable to load replies for this ticket.<a href="javascript:;" onclick="showAllReplies({{$support->ticket_id}},1)" style="color:#dd8ca0;">Click here</a> to load coments.</h5></div>');
                  });
            }
