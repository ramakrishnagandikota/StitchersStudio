        ...
        <div class="form-group row inches">
            ...
            <div class="col-sm-7">
                <select class="form-control" id="recommended_stitch_gauge_in" name="recommended_stitch_gauge_in">
                  ...
                  <option value="{{$gc1->id}}" @if($product->recommended_stitch_gauge_in == $gc1->id) selected @endif  >{{$gc1->stitch_gauge_inch .' / 1 inch'}}</option>
                  @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="form-group row inches">
            ...
            <div class="col-sm-7">
                <select class="form-control" id="recommended_row_gauge_in" name="recommended_row_gauge_in">
                  ...
                  <option value="{{$gc2->id}}" @if($product->recommended_row_gauge_in == $gc2->id) selected @endif  >{{$gc2->row_gauge_inch .' / 1 inch'}}</option>
                  @endforeach
                </select>
            </div>
        </div>
      ...
      <div class="step">
        ...
        <h4>Product Information</h4>
				...
				<div class="form-group row">
                    ...
                    <div class="col-sm-7">
                        <textarea type="text"  required id="gauge_description" class="form-control">{!! $product->gauge_description !!}</textarea>
                    </div>
                </div>
				...
				<div class="form-group row">
                    ...
                    <div class="col-sm-7">
                        <textarea type="text"  required id="material_needed" class="form-control">{!! $product->material_needed !!}</textarea>
                    </div>
                </div>
...
<table class="table table-responsive-md table-sm table-bordered" >
    ...
    <tbody id="tbody">
	  ...
	  <td><input type="number" name="min_value_inches[]" class="form-control" placeholder="Min value" value="{{ $ms->min_value_inches }}">
              <input type="number" name="max_value_inches[]" class="form-control"  placeholder="Max value" value="{{ $ms->max_value_inches }}"></td>
          <td><input type="number" name="min_value_cm[]"  class="form-control" placeholder="Min value" value="{{ $ms->min_value_cm }}">
              <input type="number" name="max_value_cm[]" class="form-control"  placeholder="Max value" value="{{ $ms->max_value_cm }}"></td>
	...
	<div class="form-group row">
		...
		<div class="col-sm-7">
		   ...
		   <?php $mname = str_replace(' ','_',$mvar->variable_name); $lname = strtolower($mname); ?>
			<option value="{{ $lname }}"  @if(count($notusedMeasurements) > 0) @foreach($notusedMeasurements as $nu) @if($lname == $nu->measurement_name) selected @endif @endforeach @endif >{{ ucfirst($mvar->variable_name) }}</option>
			@endforeach
	</select>
...
@if($product->referenceImages()->count() > 0)
          <div class="jFiler-items jFiler-row">
    <ul class="jFiler-items-list jFiler-items-grid">
      @foreach($product->referenceImages as $refpi)
        <li class="jFiler-item" data-jfiler-index="0" style="" id="refimage{{$refpi->id}}">
            <div class="jFiler-item-container">
                <div class="jFiler-item-inner">
                    <div class="jFiler-item-thumb">
                        <div class="jFiler-item-thumb-image">
                          <img src="{{$refpi->image}}" draggable="false" style="margin-top: 0px !important;">
                        </div>
                    </div>
                    <div class="jFiler-item-assets jFiler-row">
                        <ul class="list-inline pull-right">
                            <li>
                                <a class="icon-jfi-trash deleteRefImage jFiler-item-trash-action" data-id="{{$refpi->id}}"></a>
                            </li>
                        </ul>
                    </div>
                </div>
...
@endif
    ...
    <hr>
    <div class="row">
        <input type="hidden" id="paypal_verified" value="{{ $paypalDetails ? $paypalDetails->status : 1 }}" >
        @if($paypalDetails)
            <input type="hidden" name="paypal_id" id="paypal_id" value="{{ $paypalDetails->id }}">
        @else
			<input type="hidden" name="paypal_id" id="paypal_id" value="0">
		@endif
        ...
        <div class="form-group col-md-6 m-b-20">
            ...
            <select name="store_status" id="store_status" class="form-control paypal_credentials" readonly>
        ...
        <div class="form-group col-md-6 m-b-20">
            ...
            <select name="account_type" id="account_type" class="form-control paypal_credentials" readonly>
...
<script type="text/javascript">
  ...
  var URL11 = '{{url("admin/delete-pattern-reference-image")}}';
  
  var URL2 = '{{route("admin.upload.pattern.images")}}';
  var URL3 = '{{url("admin/remove-pattern-images")}}'; 
</script>
...
<script type="text/javascript">
    $(function(){
jqueryFilerReferenceImages("reference_images",URL2,URL3);
...
$(document).on('click','.deleteRefImage',function(){
  var id = $(this).attr('data-id');
  if(confirm('Are you sure want to delete this image ?')){
    $.get(URL11+'/'+id,function(res){
    if(res.status == 'success'){
      $("li#refimage"+id).remove();
    }
  });
  }
});
