...
class ProductsController extends Controller
{
    ...
    function get_products_data_all(Request $request){
    	...
    	for ($i=0; $i < count($products); $i++) {
			...
			$jsonArray[$i]['designer'] = 'knit-fit-couture';
		...
		$purchased = Booking_process::where('user_id',Auth::user()->id)->where('product_id',$products[$i]->id)->count();
...
$jsonArray[$i]['comments'][$j]['unvote_logged_in_user'] = ($userunvoteCount == 1) ? true : false;
    ...
    function delete_comment(Request $request){
        ...
        $check = ProductComments::where('user_id',Auth::user()->id)->where('id',$request->comment_id)->count();
        if($check == 0){
    ...
    function wishlist(){
        ...
        for ($i=0; $i < count($wishlist); $i++) {
            ...
            $jsonArray[$i]['image'] = $image->image_small;
