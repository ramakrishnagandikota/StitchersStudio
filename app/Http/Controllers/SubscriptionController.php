...
$subscription = Subscription::where('id',$newSubscription)->select('name','price_month','offer_price','price_year')->first();
        ...
        if($sub_type == 'yearly'){
            ...
            $data['return_url'] = url('paypal/ec-checkout-success?stype='.$sub_type.'&mode=recurring&subscription='.$newSubscription);
            $data['cancel_url'] = url('/subscription/cancel-payment?stype='.$sub_type.'&mode=recurring&subscription='.$newSubscription);
            $data['subscription_desc'] = $subscription->name.' '.$stype.' Subscription '.config('paypal.invoice_prefix').' #'.$order_id;
                ...
                $data['return_url'] = url('paypal/ec-checkout-success?stype='.$sub_type.'&subscription='.$newSubscription);
                $data['cancel_url'] = url('/subscription/cancel-payment?stype='.$sub_type.'&subscription='.$newSubscription);
            }
        }else{
            $data['items'] = [
                [
     ...
     public function getExpressCheckoutSuccess(Request $request)
    {
        ...
        if (in_array(strtoupper($response['ACK']), ['SUCCESS', 'SUCCESSWITHWARNING'])) {
            ...
            return view('subscriptions.success',compact('response','status'));
        }
    }
