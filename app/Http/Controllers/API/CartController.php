...
class CartController extends Controller
{
    ...
    function get_cart(){
        ...
        $sum=$sum+$price;
                ...
                $jsonArray[$i]['product'][$j]['images'] = $image ? $image->image_small : '';
            }
    ...
    function add_cart(Request $request){
		...
		$purchased = Orders::leftjoin('booking_process','orders.id','booking_process.order_id')
						...
						->where('booking_process.product_id',$request->product_id)
						->where('orders.order_status','Success')
						->count();
        if($purchased > 0){
            return response()->json(['status' => 'fail','message' => 'This product is already purchased.']);
            exit;
        }
    ...
    function remove_item(Request $request){
		try{
			...
			if($save){
				...
				return response()->json(['status' => 'fail',"message" => 'Unable to delete product from cart.']);
			}
		}catch(Throwable  $e){
			info($e->getMessage());
		}
